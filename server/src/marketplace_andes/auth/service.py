...
class AuthService:
    ...
    def authenticate_user(
        self, username: str, password: str, anon_ip: str
    ) -> LoginResponse:
        self.__logger.info(
            f"Authentication attempt for User: {username} at IP: {anon_ip}"
        )

        user_id = self.__user_repository.get_id_from_username(username)
        if not user_id:
            self.__logger.warning(
                f"Authentication attempted for unknown User: {username} at IP: {anon_ip}"
            )
        ...
        if not hashed_password:
            self.__logger.error(
                f"Authentication error: hashed password not found for User: {username} at IP: {anon_ip}"
            )
        ...
        try:
            ...
            self.__logger.warning(
                f"Authentication failed: invalid password for User: {username} at IP: {anon_ip}"
            )
        ...
        self.__logger.info(
            f"User: {username} authenticated successfully at IP: {anon_ip} and session_id: {session_id}"
        )
    ...
    def register_user(self, username: str, password: str, anon_ip: str) -> UUID:
        ...
        self.__session.commit()
        self.__logger.info(
            f"User: {username} registered successfully with id: {user_id} at IP: {anon_ip}"
        )
        return user_id
